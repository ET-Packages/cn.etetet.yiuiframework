    ...
    public class UIPublishModule : BaseYIUIToolModule
    {
        ...
        private void AddAllPkg()
        {
            ...
            foreach (var folder in folders)
            {
                ...
                //1 图集
                Tree.AddAllAssetsAtPath($"{m_PublishName}/{pkgName}/{YIUIConst.UIAtlasCN}",
                                        $"{m_AllPkgPath}/{pkgName}/{YIUIConst.UIAtlas}", typeof(SpriteAtlas), true, false);

                //2 预制体
                Tree.AddAllAssetsAtPath($"{m_PublishName}/{pkgName}/{YIUIConst.UIPrefabsCN}",
                                        $"{m_AllPkgPath}/{pkgName}/{YIUIConst.UIPrefabs}", typeof(UIBindCDETable), true, false);

                //3 源文件
                Tree.AddAllAssetsAtPath($"{m_PublishName}/{pkgName}/{YIUIConst.UISourceCN}",
                                        $"{m_AllPkgPath}/{pkgName}/{YIUIConst.UISource}", typeof(UIBindCDETable), true, false);

                //4 精灵
                Tree.AddAllAssetImporterAtPath($"{m_PublishName}/{pkgName}/{YIUIConst.UISpritesCN}",
                                               $"{m_AllPkgPath}/{pkgName}/{YIUIConst.UISprites}", typeof(TextureImporter), true, false);
