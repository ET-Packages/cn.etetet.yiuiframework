...
namespace YIUIFramework
{
    ...
    public sealed class UIEventHandleP2<P1, P2>
    {
        ...
        internal UIEventHandleP2<P1, P2> Init(LinkedList<UIEventHandleP2<P1, P2>> uiEventList, LinkedListNode<UIEventHandleP2<P1, P2>> uiEventNode, Entity trigger, Type onEventInvokeType)
        {
        ...
        internal bool Invoke(P1 p1, P2 p2)
        {
            if (OnEventInvokeType != null)
            {
                ...
                var iEventSystems = EntitySystemSingleton.Instance.TypeSystems.GetSystems(Trigger.GetType(), typeof(IYIUIEventInvokeSystem<P1, P2>));
                if (iEventSystems is not { Count: > 0 })
                {
                    Logger.LogError($"类:{Trigger.GetType()} UI事件名称:{OnEventInvokeType.Name} 没有具体实现的事件 IYIUIEventInvokeSystem {typeof(P1).Name} {typeof(P2).Name} 请检查");
                    return false;
                }

                foreach (IYIUIEventInvokeSystem<P1, P2> eventSystem in iEventSystems)
                {
                    if (eventSystem.GetType() == OnEventInvokeType)
                    {
                        try
                        {
                            eventSystem.Invoke(Trigger, p1, p2);
                            return true;
                        }
                        catch (Exception e)
                        {
